...
about-debugging-this-Floorp-runtime-name = この { -brand-shorter-name }
...
about-debugging-setup-this-Floorp2 = このバージョンの { -brand-shorter-name } で拡張機能と Service Worker をデバッグするには、<a>{ about-debugging-this-Floorp-runtime-name }</a> を使用します。
...
# USB section step by step guide
about-debugging-setup-usb-step-enable-debug-Floorp2 = Android 端末上の Floorp で USB デバッグを有効にします。
...
about-debugging-browser-version-too-old-fennec = このバージョンの Floorp は Android 版 Floorp (68) をデバッグできません。携帯電話でのテストには Android 版 Floorp Nightly のインストールをお勧めします。<a>詳細</a>
...
about-debugging-browser-version-too-recent = 接続したブラウザー ({ $runtimeVersion }, ビルド ID { $runtimeID }) が、実行中の { -brand-shorter-name } ({ $localVersion }, ビルド ID { $localID }) よりも新しいです。この組み合わせはサポートされてないため、開発ツールでエラーが発生する可能性があります。Floorp を更新してください。<a>トラブルシューティング</a>
