...
policy-DisableFloorpAccounts = 停用 { -fxaccount-brand-name }相關服務，包含 Sync。

# Floorp Screenshots is the name of the feature, and should not be translated.
policy-DisableFloorpScreenshots = 停用 Floorp Screenshots 功能。

policy-DisableFloorpStudies = 防止 { -brand-short-name } 進行使用者研究。
...
policy-FloorpHome = 設定 Floorp Home。
