...
is-default = Floorp for Linux では、既定のブラウザーの設定を Floorp 内部からは行えません
Linux = 既定にするには？
on-Linux= Floorp for Linux では、既定のブラウザーの設定を Floorp 内部からは行えません
how-to-set-default = 既定にするには？
is-not-default = Floorp for Linux では、既定のブラウザーの設定を Floorp 内部からは行えません
