...
about-telemetry-Floorp-data-doc = <a data-l10n-name="data-doc-link">Floorp Data Documentation</a> 當中描述了我們使用資料工具的方式。
about-telemetry-telemetry-client-doc = <a data-l10n-name="client-doc-link">Floorp Telemetry 客戶端文件</a>當中包含資料收集概念、API 文件以及資料格式的參考資料。
...
about-telemetry-show-in-Floorp-json-viewer = 用 JSON 檢視器開啟
...
# Variables:
#   $telemetryServerOwner (String): the value of the toolkit.telemetry.server_owner preference. Typically "Mozilla"
about-telemetry-origins-explanation = <a data-l10n-name="origin-doc-link">Floorp Origin Telemetry</a> 會在送出資料前為資料編碼，這樣 { $telemetryServerOwner } 才可以計數，但不知道是由哪一套 { -brand-product-name } 所送出的資料。（<a data-l10n-name="prio-blog-link">了解更多資訊</a>）
# Variables:
