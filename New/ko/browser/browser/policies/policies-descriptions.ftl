...
policy-DisableFloorpAccounts = 동기화를 포함한 { -fxaccount-brand-name } 기반의 서비스를 사용 안 합니다.

# Floorp Screenshots is the name of the feature, and should not be translated.
policy-DisableFloorpScreenshots = Floorp 스크린샷 기능을 사용 안 합니다.

policy-DisableFloorpStudies = { -brand-short-name } 연구 실행을 막습니다.
...
policy-FloorpHome = Floorp 홈을 구성합니다.
