...
about-debugging-this-Floorp-runtime-name = 這個 { -brand-shorter-name }
...
about-debugging-setup-this-Floorp2 = 於這個版本的 { -brand-shorter-name } 使用 <a>{ about-debugging-this-Floorp-runtime-name }</a> 來對擴充套件與 Service Worker 除錯。
...
# USB section step by step guide
about-debugging-setup-usb-step-enable-debug-Floorp2 = 於 Android 裝置上的 Floorp 開啟「USB 除錯」。
...
about-debugging-browser-version-too-old-fennec = 此版本的 Floorp 無法對 Floorp for Android 68 版進行除錯。我們建議您在手機上安裝 Floorp for Android Nightly 來進行測試。<a>更多詳情</a>
...
about-debugging-browser-version-too-recent = 連結的瀏覽器（版本 { $runtimeVersion }、buildID { $runtimeID }）比您的 { -brand-shorter-name }（{ $localVersion }、buildID { $localID }） 還新。這是不支援的設定，可能會造成開發者工具發生錯誤，請更新 Floorp。<a>點此進行疑難排解</a>
