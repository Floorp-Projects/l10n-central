...
ssl-connection-error = { $hostname } への接続中にエラーが発生したで。{ $errorMessage }
# Variables:
# $error (string) - NSS error code string that specifies type of cert error. e.g. unknown issuer, invalid cert, etc.
cert-error-code-prefix = エラーコード: { $error }
psmerr-ssl-disabled = SSL プロトコルが無効になっているため、安全な接続ができへんかった。
psmerr-ssl2-disabled = サイトが古くて安全でないバージョンの SSL プロトコルを使用しているため、安全な接続ができへんかった。
# This is a multi-line message.
psmerr-hostreusedissuerandserial =
    無効な証明書を受信したで。サーバー管理者またはメール送信者に次の情報を知らせてや:

    あなたのサーバー証明書は認証局によって発行された他の証明書と同じシリアル番号を持っています。一意なシリアル番号を持つ新しい証明書を取得してや。
