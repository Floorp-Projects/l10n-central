...
containers-remove-alert-msg = このコンテナーを消したら { $count } 個のコンテナータブが閉じらレルで。このコンテナーを消してもええんか？
...
home-prefs-content-description = Floorp Home に表示しとくコンテンツを選ぼうや。
...
home-prefs-snippets-header =
    .label = スニペット
home-prefs-snippets-description-new = { -vendor-short-name } と { -brand-product-name } に関しとくヒントと最新情報
...
sync-signedout-description2 = ブックマークやページの表示履歴、タブ、パスワード、アドオン、設定を、お持ちの全部の端末間で同期でくで。
...
sync-manage-account = アカウントの管理
    .accesskey = o
sync-signedin-unverified = { $email } は認証されていへん。
...
save-files-to-cloud-storage =
    .label = { $service-name } にファイルを保存しとく

# Floorp の固有設定

notice-from-floorp-projects = { -brand-short-name } のポリシー管理に関しとく通知
...
preferences-browser-appearance-description  = { -brand-short-name } の外観を設定すんで。ここで設定を変更しても、テーマの設定には影響しへん。ただし、テーマによっては互換性がへん場合があります。
...
move-tabbar-position = タブバーの位置をブックマークの下に移動しとく
...
about-uboori = uBlock Origin は広告やトラッカー、悪意あるサイトや詐欺からユーザーを保護すんで。一般的なものよりも軽量やで。

Privacy-Badger = Privacy Badger
about-PBadger = Privacy Badger は、非表示のトラッカーをブロックしとくことを学習して、それに基づいてトラッカーを能動的にブロックすんで。

Duck-Duck-Go = DuckDuckGo Privacy Essentials
about-DDG = 検索エンジンを DuckDuckGo に置き換え、訪問したサイトのトラッカーをブロックし、そのサイトを評価すんで。
...
sidebar2-restore =
 .label = { -brand-short-name } の再起動・新しいウインドウを開いたときに、ブラウザーマネージャーサイドバーの内容を復元しとく
