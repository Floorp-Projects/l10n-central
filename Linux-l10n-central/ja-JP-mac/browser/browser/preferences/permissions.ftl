...
permissions-exceptions-https-only-window =
    ...
    .style = { permissions-window.style }
permissions-exceptions-https-only-desc = HTTPS-Only モードをオフにするウェブサイトを指定できます。{ -brand-short-name } はこれらのサイトでは安全な接続にアップグレードしません。プライベートウインドウではこの例外は適用されません。
...
permissions-site-autoplay-window =
    .title = 設定 - 自動再生
    .style = { permissions-window.style }
permissions-site-autoplay-desc = 自動再生のデフォルト設定によらないサイトをここで管理できます。
